...
void radixSortBlockKeysOnly(uint4 *key, uint4 *val, uint nbits, uint startbit, __local uint* sMem, __local uint* numtrue)
{
	int localId = get_local_id(0);
	int localSize = get_local_size(0);
	...
	for(uint shift = startbit; shift < (startbit + nbits); ++shift)
	{
		...
		// This arithmetic strides the ranks across 4 CTA_SIZE regions
		sMem[(r.x & 3) * localSize + (r.x >> 2)] = (*val).x;
		sMem[(r.y & 3) * localSize + (r.y >> 2)] = (*val).y;
		sMem[(r.z & 3) * localSize + (r.z >> 2)] = (*val).z;
		sMem[(r.w & 3) * localSize + (r.w >> 2)] = (*val).w;
		barrier(CLK_LOCAL_MEM_FENCE);

		// The above allows us to read without 4-way bank conflicts:
		(*val).x = sMem[localId];
		(*val).y = sMem[localId +     localSize];
		(*val).z = sMem[localId + 2 * localSize];
		(*val).w = sMem[localId + 3 * localSize];
...
__kernel void radixSortBlocksKeysOnly(__global uint4* keysIn, 
				      ...
				      __global uint4* valsIn, 
				      __global uint4* valsOut,
				      uint nbits,
				      uint startbit,
				      uint numElements, 
				      uint totalBlocks,
...

{
	...
	radixSortBlockKeysOnly(&key, &val, nbits, startbit, sMem, numtrue);
	
	keysOut[globalId] = key;
	valsOut[globalId] = val;
}
  ...
  if (globalOffset < numElements)
    {
      ...
      outVals[globalOffset]   = sVals1[localId];
    }
  ...
  if (globalOffset < numElements)
    {
      ...
      outVals[globalOffset]   = sVals1[localId + groupSize];
    }
